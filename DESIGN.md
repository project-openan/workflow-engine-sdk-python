...
- **`WorkflowEngineClient`** - the workflow execution send path. Owns Task-T
  ...
  receive), the global `EventCallback`, and the `ControlPoint` wiring.
...
via `ExtensionSender` before the workflow starts, not in-workflow callbacks.
...
2. **In-workflow vs pre-positioning extensions** - Task-T and Negotiation-T
